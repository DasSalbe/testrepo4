...
`;
    ...
    */
    ...
    function syncMobilePlayerPosition() {
        if (!musicPlayer) return;

        if (window.innerWidth <= 768) {
            musicPlayer.classList.add("mobile-floating");
            musicPlayer.style.position = "fixed";
            musicPlayer.style.left = "50%";
            musicPlayer.style.bottom = `calc(12px + env(safe-area-inset-bottom))`;
            musicPlayer.style.transform = "translateX(-50%)";
            musicPlayer.style.zIndex = "900";
        } else {
            musicPlayer.classList.remove("mobile-floating");
            musicPlayer.style.position = "";
    ...
    syncMobilePlayerPosition();
    window.addEventListener("resize", syncMobilePlayerPosition);
    window.addEventListener("orientationchange", syncMobilePlayerPosition);
    window.addEventListener("scroll", syncMobilePlayerPosition, { passive: true });
    ...
    function playLoadedSong() {
        music.play().then(() => {
            isPlaying = true;
            if (playBtn) playBtn.textContent = "âšâš";
        }).catch(() => {
            isPlaying = false;
...
function typeText() {
    ...
    function createFlowerHeart() {
        ...
        const heartBox = container.getBoundingClientRect();
		const centerX = isMobile ? heartBox.width / 2 : 350;
		const centerY = isMobile ? heartBox.height * 0.49 : 320;

		const scale = isMobile ? Math.min(heartBox.width, heartBox.height) / 36 : 16;
    ...
    if (songSelect) songSelect.addEventListener("change", () => {
        currentSongIndex = Number(songSelect.value);
        loadSong(currentSongIndex);
        playLoadedSong();
    });

    if (playerToggle) playerToggle.addEventListener("click", () => {
        const expanded = musicPlayer.classList.toggle("expanded");
        playerToggle.setAttribute("aria-expanded", String(expanded));
        playerToggle.textContent = expanded ? "Less" : "More";
    });

    if (musicPlayer) musicPlayer.addEventListener("click", (event) => {
        if (window.innerWidth > 768 || musicPlayer.classList.contains("expanded")) return;
        if (event.target.closest("button, input, select")) return;

        musicPlayer.classList.add("expanded");
        if (playerToggle) {
            playerToggle.setAttribute("aria-expanded", "true");
            playerToggle.textContent = "Less";
        }
    });
